...
void aloca_matriz(int l, int c, int*** grid, int*** newgrid){
  ...
  *newgrid = malloc(l * sizeof(int*));
  #pragma omp parallel private(i,j) shared(grid,newgrid,l,c)
  {
  #pragma omp for
  for(i=0; i<l; i++){
...
void nova_geracao(int*** grid, int*** newgrid, int linhas, int colunas){
  int vivos,i,j;
  #pragma omp parallel private(i,j,vivos) shared(grid,newgrid,linhas,colunas) 
  {
  #pragma omp for collapse(2)
  for(i=0;i<linhas;i++){
    for(j=0;j<colunas;j++){
...
int soma_celulas(int*** grid, int linhas, int colunas){
  int i,j;
  int nthreads;
  #pragma omp parallel private(i,j) shared(grid,linhas,colunas,soma)
  {
  #pragma omp for collapse(2)
  for(i=0;i<linhas;i++){
    for(j=0;j<colunas;j++){
...
int main(void) {
  ...
  printf("tempo do trecho de código do somatório de células vivas da última geração - openmp: %lld  ms\n", tmili);
}